...
const FIELD_MAPPINGS = {
  ...
  [IP_API_TYPES.IP_API_COM]: {
    ...
    country_flag: data => `https://flagcdn.com/w80/${data.countryCode?.toLowerCase()}.png`,
    // 代理相关字段
    isp: 'isp',
    org: 'org',
    as: 'as',
  ...
  [IP_API_TYPES.IPINFO_IO]: {
    ...
    country_flag: data =>
      data.country ? `https://flagcdn.com/w80/${data.country.toLowerCase()}.png` : '',
    // 代理相关字段
    org: 'org',
    hostname: 'hostname',
    // ipinfo.io 付费版提供 privacy 字段
    is_proxy: data => data.privacy?.proxy || false,
    is_hosting: data => data.privacy?.hosting || false
  },
  ...
  [IP_API_TYPES.IPAPI_CO]: {
    ...
    country_flag: data =>
      data.country_flag || `https://flagcdn.com/w80/${data.country_code?.toLowerCase()}.png`,
    // 代理相关字段
    isp: 'org',
    org: 'org',
    as: 'asn',
    is_proxy: data => data.threat?.is_proxy || false,
    is_hosting: data => data.threat?.is_datacenter || false
  },
  ...
  [IP_API_TYPES.IP_GEOLOCATION]: {
    ...
    languages: data => data.languages?.split(',')[0] || 'en-US',
    country_flag: data =>
      data.country_flag || `https://flagcdn.com/w80/${data.country_code2?.toLowerCase()}.png`,
    // 代理相关字段
    isp: 'isp',
    org: 'organization',
    as: 'asn',
    is_proxy: data => data.security?.is_proxy || false,
    is_hosting: data => data.security?.is_datacenter || false
  },
...
export function normalizeIpGeoData(rawData, apiType = IP_API_TYPES.CUSTOM) {
  ...
  const normalized = {
    ...
    // 代理相关字段
    isp: getFieldValue(rawData, mapping.isp) || '',
    org: getFieldValue(rawData, mapping.org) || '',
    as: getFieldValue(rawData, mapping.as) || '',
    is_proxy: getFieldValue(rawData, mapping.is_proxy) || false,
    is_hosting: getFieldValue(rawData, mapping.is_hosting) || false
  }
...
export function generateBrowserConfig(geoData) {
  return {
    ...
    proxy_info: {
      ...
      org: geoData.org || '',
      as: geoData.as || '',
      is_proxy: geoData.is_proxy || false,
